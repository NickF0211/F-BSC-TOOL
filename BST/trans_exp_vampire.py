...
def run_exp(command_header):
    ...
    with open("trans_rule_template_vampire.txt", 'r') as rule_template:
    ...
    rule_file = "trans_req_rule_unbound_vampire.py"
    with open(outfile, 'w') as out_f:
    ...
    for j in range(1, 4):
        ...
        result_file = "results/trans_unbound_rule_{}_vampire.txt".format(j)
        print(result_file)
        with open(result_file, 'w') as f:
            try:
