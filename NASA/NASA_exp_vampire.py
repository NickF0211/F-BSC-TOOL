...
def run_exp(command_hear):
    ...
    timeout = 5000
    #init value
    rule_file  = "verification_rules_vampire.py"
    for i in range(1,3):
        ...
        for j in range(1, 7):
            result_file = "results/NASA_{}_rule_{}_vampire.txt".format(i, j)
            print(result_file)
            with open(result_file, 'w') as f:
                try:
                    result = subprocess.run(command_hear + [ rule_file, str(j), "f", "f", "t"], stdout=subprocess.PIPE, stderr=subprocess.PIPE,
