...
def run_exp(command_header):
    ...
    for j in range(7, 11):
        ...
        with open(result_file, 'w') as f:
            try:
                result = subprocess.run(command_header + [ rule_file, j, "f"], stdout=subprocess.PIPE,
        ...
        with open(result_file, 'w') as f:
            try:
                result = subprocess.run(command_header + [rule_file, j, "f", "f", "t"], stdout=subprocess.PIPE,
                                        stderr=subprocess.PIPE,
                                        universal_newlines=True,
                                        timeout=timeout)
                f.write(result.stdout)
